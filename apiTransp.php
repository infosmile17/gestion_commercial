...
if (isset($_POST['enregistrer_modification']) && $_POST['enregistrer_modification'] != '') {
    ...
    $sql = "UPDATE `transp` SET `nomprenom`='$nomprenom',`tel1`='$tel1',`tel2`='$tel2',`email`='$email',`adresse`='$adresse',`remarques`='$remarques' WHERE `id` = '{$id}' LIMIT 1";

    if ($con->query($sql) === TRUE) {
        $msg = "Modification avec succée du transporteur";
        header("location: transporteur.php?msg=" . $msg);
    } else {
...
if (isset($_POST['enregistrer']) && $_POST['enregistrer'] != '') {
    ...
    $sql = "INSERT INTO transp (nomprenom, tel1, tel2, email, adresse, remarques) VALUES ('$nomprenom', '$tel1', '$tel2', '$email', '$adresse', '$remarques')";
    if ($con->query($sql) === TRUE) {
        $msg = "Nouveau transporteur créé avec succès";
        header("location: transporteur.php?msg=" . $msg);
    } else {
        header("location: 404.php");
    }
    var_dump($_POST);
} elseif (isset($_POST['id_sup']) && isset($_POST['Supprimer_transporteur'])) {
    if ($_POST['id_sup'] != '') {
        $id = $_POST['id_sup'];
        $sql = "DELETE FROM `transp` WHERE `id` ='{$id}' LIMIT 1";
        if ($con->query($sql) === TRUE) {
            $msg = "Suppression d'un transporteur avec succès";
            header("location: transporteur.php?msg=" . $msg);
        } else {
            header("location: 404.php");
        }
...
?>
        ...
        echo ("<script src='script_transp.js'></script>");
    } else {
